-- ╔══════════════════════════════════════════════════════════════╗
-- ║      CLOWN HUB — Blox Fruits Working Engine (Bypass)         ║
-- ╚══════════════════════════════════════════════════════════════╝
...
-- ══ РАБОЧИЕ ФУНКЦИИ ДВИЖКА ════════════════════════════════════

-- 1. Экипировка первого попавшегося оружия из инвентаря
...
-- 2. Безопасное перемещение (Обход античита через Tween)
...
-- 4. Рабочая система нанесения урона
local function performAttack()
    equipWeapon()
    
    -- Симуляция клика через VirtualUser и сработку аниматора
    local vUser = game:GetService("VirtualUser")
    vUser:CaptureController()
    vUser:ClickButton1(Vector2.new(500, 500))
...
local C = {
    ...
    t2  = Color3.fromRGB(185, 185, 200), t3  = Color3.fromRGB(120, 120, 135),
    grn = Color3.fromRGB(46, 204, 113), white = Color3.fromRGB(255, 255, 255),
}
...
pagesContainer.Size = UDim2.new(1, -170, 1, 0)
pagesContainer.Position = UDim2.new(0, 170, 0, 0)
...
makeSwitch(combatPage, "Kill Aura (Аулы вокруг)", Logic.State.KillAura, function(on) Logic.State.KillAura = on end)
...
print("[CLOWN HUB]: Operational engine running!")
